...
static inline int
add_bkpt (Dwarf_Addr **bkpts, int nbkpts, Dwarf_Addr pc)
{
...
static inline int
entrypc_bkpt (Dwarf_Die *die, Dwarf_Addr **bkpts, int nbkpts)
{
  Dwarf_Addr pc;
...
static inline int
search_range (Dwarf_Lines *lines, size_t nlines,
              Dwarf_Addr **bkpts, int nbkpts,
     ...
     Dwarf_Addr **bkpts;
{
  ...
  *bkpts = NULL;
  ...
  if (offset == 1)
    return search_range (lines, nlines, bkpts, nbkpts, begin, end, true, true) ?: entrypc_bkpt (die, bkpts, nbkpts);
  ...
  while (offset > 0)
    {
      ...
      if (search_range (lines, nlines, bkpts, nbkpts, begin, end, true, false) < 0)
	return -1;
  ...
  return (nbkpts
	  ?: (lowpc == (Dwarf_Addr) -1l ? 0
	      : search_range (lines, nlines, bkpts, nbkpts, lowpc, highpc, false, true))
